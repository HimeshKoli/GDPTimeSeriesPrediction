...
# line15>> passed argument in date format because our index is financial_year which is in date format
# line18>> start and end takes range so starting from len(preprocessed_gdp_df) i.e: 62 which coordinates to that year
# in Financial_year, and it takes that value from index till end=year which we will give at the end of our choice
# line21&22>> create new .pkl file and load our data in that file
# line24>> at the end we will get range between start and end, but we need only last value in that series range, so we
...
# This whole function bracket is designed to work to get a range on user input, so since we have already implemented one
# type we will see this implementation in future
